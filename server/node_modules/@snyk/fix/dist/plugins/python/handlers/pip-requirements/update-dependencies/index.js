...
function updateDependencies(parsedRequirementsData, updates, directUpgradesOnly = false, referenceFileInChanges) {
    ...
    const { updatedRequirements, changes: upgradedChanges, appliedRemediation, } = generate_upgrades_1.generateUpgrades(requirements, updates, referenceFileInChanges);
    debug('Finished generating upgrades to apply');
    let pinnedRequirements = [];
    let pinChanges = [];
    let appliedPinsRemediation = [];
    if (!directUpgradesOnly) {
        ({
            pinnedRequirements,
            changes: pinChanges,
            appliedRemediation: appliedPinsRemediation,
        } = generate_pins_1.generatePins(requirements, updates, referenceFileInChanges));
